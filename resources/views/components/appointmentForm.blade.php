...
<form id="appointment-form" class="flex flex-col md:flex-row justify-between md:items-center mt-6 gap-4" method="POST"
    ...
    <div class="flex-1">
        <x-input-label for="title">
            Bezeichnung
            <x-text-input id="title" type="text" name="title" required minlength="4" maxlength="50" size="55" placeholder="Bezeichnung" class="border border-gray-400"></x-text-input>
        </x-input-label>
...
@if (session('success'))
    <div id="success-alert" class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mt-5" role="alert">
        <strong class="font-bold">Erfolgreich gespeichert!</strong>
        <span class="block sm:inline">{{ session('success') }}</span>
        <span class="close absolute top-0 bottom-0 right-0 px-4 py-3">
            <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                <title>Close</title>
                <path fill-rule="evenodd"
                      d="M14.95 5.05a.75.75 0 0 1 1.06 1.06L11.06 10l4.95 4.95a.75.75 0 0 1-1.06 1.06L10 11.06l-4.95 4.95a.75.75 0 0 1-1.06-1.06L8.94 10 4.99 5.05a.75.75 0 0 1 1.06-1.06L10 8.94l4.95-4.95z"></path>
            </svg>
        </span>
    </div>
    <script>
